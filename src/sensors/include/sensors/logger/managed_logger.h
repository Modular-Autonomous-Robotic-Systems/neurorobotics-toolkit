...
class ManagedLogger : public rclcpp_lifecycle::LifecycleNode {
  public:
     ...
     * @param options The NodeOptions for this lifecycle node, passed from the
     * component manager.
     */
    explicit ManagedLogger(const rclcpp::NodeOptions &options);
    ~ManagedLogger();

    std::shared_ptr<rosbag2_transport::Recorder> GetRecorderNode() {
        return mpRecorder;
    }
  ...
  private:
    ...
    /**
     * @brief Checks if the provided bagfile path exists and updates the output
     * file path with an incremental ID
     */
    ...
    /// @brief Client to call the change state service.
    rclcpp::Client<lifecycle_msgs::srv::ChangeState>::SharedPtr mpChangeStateClient;
};
