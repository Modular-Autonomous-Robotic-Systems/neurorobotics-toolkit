...
ManagedLogger::ManagedLogger(const rclcpp::NodeOptions &options)
    ...
    mpChangeStateClient = this->create_client<lifecycle_msgs::srv::ChangeState>(
        std::string(this->get_name()) + "/change_state");
}

ManagedLogger::~ManagedLogger() {
    RCLCPP_INFO(this->get_logger(), "Destructor for ManagedLoggerCalled");
    if (mpChangeStateClient) {
        auto request =
        ...
        auto future_deactivate =
            mpChangeStateClient->async_send_request(request);
        // Wait safely for a short period
        if (future_deactivate.wait_for(std::chrono::seconds(2)) ==
            std::future_status::ready) {
            future_deactivate.get();
        }

        // Cleanup
        request->transition.id =
            lifecycle_msgs::msg::Transition::TRANSITION_CLEANUP;
        auto future_cleanup = mpChangeStateClient->async_send_request(request);
        if (future_cleanup.wait_for(std::chrono::seconds(2)) ==
            std::future_status::ready) {
            future_cleanup.get();
        }
    }
...
CallbackReturn ManagedLogger::on_configure(const rclcpp_lifecycle::State &) {
    RCLCPP_INFO(this->get_logger(),
                "In on_configure, configuring the recorder...");
    try {
        InitializeParameters();
    } catch (const std::runtime_error &e) {
        RCLCPP_FATAL(this->get_logger(), "Error during configuration: %s",
                     e.what());
    ...
    for (int i = 0; i < mvpTopicsToRecord.size(); i++) {
        RCLCPP_INFO(this->get_logger(), "Topic %d: %s", i,
                    mvpTopicsToRecord[i].c_str());
    }
    mpRecordOptions->rmw_serialization_format =
    ...
    // 	mpRecordOptions->topic_qos_profile_overrides.emplace(topic_name,
...
CallbackReturn ManagedLogger::on_activate(const rclcpp_lifecycle::State &) {
    RCLCPP_INFO(this->get_logger(),
                "In on_activate, activating the recorder...");
    ...
    try {
        ...
        mpRecorder = std::make_shared<rosbag2_transport::Recorder>(
            mpWriter, *mpStorageOptions, *mpRecordOptions, recorder_name,
            options);
        mpRecorderExecutor =
            std::make_shared<rclcpp::executors::MultiThreadedExecutor>();
        mpRecorderExecutor->add_node(mpRecorder);
        mpRecordThread = std::make_shared<std::thread>(
            [this]() { mpRecorderExecutor->spin(); });
        mpRecorder->record();
    } catch (const std::exception &e) {
        RCLCPP_FATAL(this->get_logger(), "Error during activation: %s",
                     e.what());
        if (mpRecorder) {
            mpRecorder->stop();
        }
        if (mpRecorderExecutor) {
            // 1. Cancel the executor, which will stop the spin() call in the
            // thread.
        ...
        if (mpRecorder && mpRecorderExecutor) {
            // 3. Remove the node from the executor to prepare for a potential
            // reactivation.
    ...
    RCLCPP_INFO(this->get_logger(),
                "Activation successful. Recording started.");
    return CallbackReturn::SUCCESS;
}

CallbackReturn ManagedLogger::on_deactivate(const rclcpp_lifecycle::State &) {
    RCLCPP_INFO(this->get_logger(),
                "In on_deactivate, stopping the recorder...");
    if (mpRecorder) {
        mpRecorder->stop();
    }
    if (mpRecorderExecutor) {
    ...
    RCLCPP_INFO(this->get_logger(),
                "Deactivation successful. Recording stopped.");
    return CallbackReturn::SUCCESS;
}
        ...
        if (last_underscore_pos == std::string::npos) {
            ...
            try {
                std::string number_str =
                    filepath.substr(last_underscore_pos + 1);
                int current_number = std::stoi(number_str);
                int new_number = current_number + 1;
                // Create the new filename by replacing the old number with the
                // new one.
                filepath = filepath.substr(0, last_underscore_pos + 1) +
                           std::to_string(new_number);
            } catch (const std::invalid_argument &ia) {
                // This catch block handles cases where the substring after '_'
                // is not a valid number. For example, "/ws/data/telemetry_". In
                // this case, we can append "1".
                filepath += "1";
            } catch (const std::out_of_range &oor) {
                // This handles cases where the number is too large to fit in an
                // int.
                RCLCPP_ERROR(this->get_logger(),
                             "Error: Number out of range in filename: %s",
                             filepath.c_str());
            }
        }
        if (!UpdateFilenameIfExists(filepath)) {
            RCLCPP_INFO(this->get_logger(),
                        "Provided filename exists, logging at: %s",
                        filepath.c_str());
        }
        return true;
    } else {
...
void ManagedLogger::InitializeParameters() {
    ...
    if (std::filesystem::exists(mpOutputBagName)) {
        RCLCPP_WARN(this->get_logger(),
                    "bag with specified name exists, adding a number in the "
                    "name to distinguish");
    }

    if (mvpTopicsToRecord.empty()) {
        RCLCPP_ERROR(this->get_logger(),
                     "Parameter 'topics_to_record' is not set or is empty.");
    }
}
